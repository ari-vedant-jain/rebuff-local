...
from rebuff.detect_pi_vectorbase import init_pinecone, init_chroma
...
class Rebuff:
    def __init__(self, api_token: str, api_url: str = "https://playground.rebuff.ai", use_chroma: bool = False, chroma_collection: Optional[str] = None):
    ...
    def initialize_vector_store(self, pinecone_apikey: Optional[str] = None, pinecone_index: Optional[str] = None, use_ollama: bool = True) -> None:
        """Initializes either Pinecone or Chroma vector store depending on the configuration."""
        if self.use_chroma:
            if not self.chroma_collection:
                raise ValueError("Chroma collection name must be provided if use_chroma is set to True.")
            self.vector_store = init_chroma(self.chroma_collection)
        else:
            if not pinecone_apikey or not pinecone_index:
                raise ValueError("Pinecone API key and index are required for Pinecone initialization.")
            self.vector_store = init_pinecone(api_key=pinecone_apikey, index=pinecone_index, use_ollama=use_ollama)
    ...
    def detect_injection(
        ...
        if check_vector and not self.vector_store:
            raise ValueError("Vector store is not initialized. Please initialize it before running vector checks.")
